...
import (
	...
	mockgateway "my_app/internal/gateway/mock"
	lg "my_app/internal/logger"
	"my_app/internal/models"
	"my_app/internal/pagination"
	mockservice "my_app/internal/service/mock"
	"my_app/pkg/responce_errors"
	"testing"
)
...
func TestCommentsService_CreateComment(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			args:      args{comment: defaultComment},
			want:      FuncRes{com: models.Comment{}, err: responsce_error.ResponseError{}},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			defer ctl.Finish()
			repo := mockgateway.NewMockComments(ctl)
			getter := mockservice.NewMockPostGetter(ctl)
			if !tt.repoRes.skip {
